...
function CheckboxDataFactory(checkbox, onChange, visible) {
    if (checkbox != null)
        return {
            visible: Helpers_1.defVal(checkbox.visible, visible),
            checked: Helpers_1.defVal(checkbox.checked, false),
            onChange: onChange,
            childrenCheckedCount: 0
        };
    else
        return { visible: visible, checked: false, onChange: onChange, childrenCheckedCount: 0 };
}
...
ReactDOM.render(React.createElement(Tree_1.Tree, { checkboxes: false, tree: {
        text: "Root",
        nodes: [
            { text: "First node" },
...
var Tree = /** @class */ (function (_super) {
    ...
    function Tree(props) {
        ...
        _this.handleOpenChange = function (id, opened) {
            var node = _this.nodeSelector(id);
            node.opened = opened;
            _this.setState({ node: _this.node });
        };
    ...
    Tree.prototype.initList = function (node) {
        node.id = "0";
        node.checkbox = Checkbox_1.CheckboxDataFactory(node.checkbox, this.handleCheckboxChange, this.props.checkboxes);
    ...
    Tree.prototype.render = function () {
        return (React.createElement("div", null,
            React.createElement(Item_1.Item, { key: this.state.node.id, id: this.state.node.id, text: this.state.node.text, nodes: this.state.node.nodes, opened: this.state.node.opened, onOpen: this.state.node.onOpen, checkbox: this.state.node.checkbox })));
    };
    return Tree;
}(React.Component));
exports.Tree = Tree;
...
var Helpers_1 = __webpack_require__(1);
var Checkbox_1 = __webpack_require__(2);
var OpenButton_1 = __webpack_require__(7);
function NodePropsFactory(node) {
    var nodes = [];
    if (node.nodes != null)
        for (var i = 0; i < node.nodes.length; i++) {
            node.nodes[i].checkbox = Checkbox_1.CheckboxDataFactory(node.nodes[i].checkbox, node.checkbox.onChange, node.checkbox.visible);
...
var Item = /** @class */ (function (_super) {
    ...
    Item.prototype.handleOpenChange = function (event) {
        var target = event.target;
    ...
    Item.prototype.renderSublist = function () {
        if (this.props.nodes && this.props.opened) {
            ...
            for (var i = 0; i < this.props.nodes.length; i++) {
                nodes.push(React.createElement(Item, { key: this.props.nodes[i].id, id: this.props.nodes[i].id, text: this.props.nodes[i].text, nodes: this.props.nodes[i].nodes, opened: this.props.nodes[i].opened, onOpen: this.props.nodes[i].onOpen, checkbox: this.props.nodes[i].checkbox }));
            }
            return nodes;
        }
        else
    ...
    Item.prototype.render = function () {
        var checkbox = this.props.checkbox.visible ? (React.createElement(Checkbox_1.Checkbox, { onChange: this.handleCheckChange, checked: this.props.checkbox.checked })) : null;
        var openButton = this.props.nodes.length > 0 ? (React.createElement(OpenButton_1.OpenButton, { onChange: this.handleOpenChange, opened: this.props.opened })) : null;
...
var OpenButton = /** @class */ (function (_super) {
    ...
    OpenButton.prototype.render = function () {
        return (React.createElement("input", { type: "checkbox", checked: this.props.opened, onChange: this.props.onChange }));
    };
    return OpenButton;
}(React.Component));
exports.OpenButton = OpenButton;
