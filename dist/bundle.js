...
var SelectButton_1 = __webpack_require__(6);
var ExpandButton_1 = __webpack_require__(8);
var Tree = /** @class */ (function (_super) {
    ...
    function Tree(props) {
         ...
         */
        _this.handleSelectButtonChange = function (checked, id) {
            var node = _this.nodeSelector(id);
            _this.nodeSelectButtonChange(checked, node, true);
         ...
         */
        _this.handleExpandedChange = function (id, expanded) {
            var node = _this.nodeSelector(id);
    ...
    Tree.prototype.initList = function (node) {
        node.id = "0";
        node.state = Node_1.NodeStateFactory(node.state);
        node.state.expanded = true;
        node.checkbox = SelectButton_1.SelectButtonDataFactory(node.state.checked, this.handleSelectButtonChange);
        node.expandButton = ExpandButton_1.ExpandButtonDataFactory(node.state.expanded, this.handleExpandedChange);
        node = Node_1.NodePropsFactory(node);
        return node;
    };
    /**
     ...
     */
    Tree.prototype.parentSelectButtonChange = function (checked, node) {
        // Root node:
        if (node.id === "0")
            return;
        // Others:
        ...
        var state = SelectButton_1.SelectButtonState.Unselected;
        var checked_counter = 0;
        for (var i = 0; i < parentNode.nodes.length; i++) {
            var curr_state = parentNode.nodes[i].checkbox.checked;
            if (curr_state === SelectButton_1.SelectButtonState.PartiallySelected) {
                state = SelectButton_1.SelectButtonState.PartiallySelected;
                break;
            }
            else if (curr_state === SelectButton_1.SelectButtonState.Selected)
                checked_counter++;
        }
        if (state === SelectButton_1.SelectButtonState.Unselected)
            if (checked_counter === parentNode.nodes.length)
                state = SelectButton_1.SelectButtonState.Selected;
     ...
     */
    Tree.prototype.nodeSelectButtonChange = function (checked, node, directlyChanged) {
        if (directlyChanged === void 0) { directlyChanged = false; }
        if (node.nodes) {
            node.checkbox.checked = checked ? SelectButton_1.SelectButtonState.Selected : SelectButton_1.SelectButtonState.Unselected;
 ...
 * @constructor
 */
function NodeStateFactory(state, parentState) {
    if (parentState === void 0) { parentState = null; }
    if (state != null && parentState != null) {
        return {
            checked: Helpers_1.defVal(state.checked, parentState.checked),
            disabled: Helpers_1.defVal(state.disabled, parentState.disabled),
            expanded: Helpers_1.defVal(state.expanded, false),
            selected: Helpers_1.defVal(state.selected, parentState.selected)
        };
    }
    else if (state != null && parentState == null) {
        return {
            checked: Helpers_1.defVal(state.checked, false),
            disabled: Helpers_1.defVal(state.disabled, false),
            expanded: Helpers_1.defVal(state.expanded, false),
            selected: Helpers_1.defVal(state.selected, false)
        };
    }
    else if (state == null && parentState != null)
        return {
...
exports.NodeStateFactory = NodeStateFactory;
/**
 * TODO Commentary
 *
 * @param {NodeProps} node
...
function NodePropsFactory(node) {
    ...
    if (node.nodes != null)
        for (var i = 0; i < node.nodes.length; i++) {
            ...
            current.checkbox = SelectButton_1.SelectButtonDataFactory(current.state.checked, node.checkbox.onChange);
            current.checkable = Helpers_1.defVal(current.checkable, node.checkable);
            current.expandButton = ExpandButton_1.ExpandButtonDataFactory(current.state.expanded, node.expandButton.onChange);
...
var Node = /** @class */ (function (_super) {
     ...
     */
    Node.prototype.handleCheckChange = function (checked) {
        this.props.checkbox.onChange(checked, this.props.id);
    };
    Node.prototype.handleOpenChange = function (expanded) {
        this.props.expandButton.onChange(this.props.id, expanded);
    };
    /**
     * @returns {JSX.Element[]} The rendered nodes.
     */
    Node.prototype.renderSublist = function () {
        if (this.props.nodes && this.props.state.expanded) {
            var nodes = [];
            for (var i = 0; i < this.props.nodes.length; i++) {
                nodes.push(React.createElement(Node, { key: this.props.nodes[i].id, id: this.props.nodes[i].id, text: this.props.nodes[i].text, nodes: this.props.nodes[i].nodes, state: this.props.nodes[i].state, expandButton: this.props.nodes[i].expandButton, checkbox: this.props.nodes[i].checkbox, checkable: this.props.nodes[i].checkable }));
            }
            return nodes;
        }
        else
...
var FaSquare = function FaSquare(props) {
    return _react2.default.createElement(
        _reactIconBase2.default,
        _extends({ viewBox: '0 0 40 40' }, props),
        _react2.default.createElement(
